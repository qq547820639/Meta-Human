...
## 阶段七：P1 零配置首次使用
- [ ] Task 8: 普通用户首次使用
  - [ ] 自动检测架构/内存/磁盘/麦克风/摄像头；自动发现本地模型服务
  - [ ] 完全本地/云端增强/混合三预设；高级配置折叠到开发者设置
  - [ ] 开箱即用示例数字人与示例对话
  - [ ] 分步引导（环境→形象→声音→首次回答→记忆）；一次只做一件事
  - [ ] 每个失败态提供检测/原因/影响/一键修复；预计进度/可取消/后台继续/恢复
  - [ ] 首次成功标准：用户获得一条有意义且可听见的回答

## 阶段八：P1 知识/引用/记忆可信度
- [ ] Task 9: 引用与来源可信度
  - [ ] 引用原文预览、定位具体段落、来源更新时间、同步状态
  - [ ] 失效/冲突来源提示、检索分数与重排证据仅高级模式展示
  - [ ] 无可靠依据明确回答不知道；知识库重建/暂停/恢复/增量同步
  - [ ] 文档删除后引用失效处理；用户可对引用纠错
- [ ] Task 10: 记忆可信度
  - [ ] 记忆候选而非静默保存；确认/编辑/拒绝
  - [ ] 作用域（数字人/会话/用户）、有效期、敏感级别、自动脱敏
  - [ ] 查看某回答用了哪些记忆；一键遗忘；批量导出/导入
  - [ ] 临时会话绝不进长期记忆；删除数字人后本地/远程记忆清理可验证
  - [ ] 离线评测集：准确性/冲突/过期/隐私泄漏/跨会话污染

## 阶段九：P1 隐私/费用/数据流透明
- [ ] Task 11: 隐私与费用透明
  - [ ] 「禁止所有远程 Provider」总开关、每 Provider 独立开关
  - [ ] 每轮数据流向提示（照片/音频/文本/文档类型）
  - [ ] 首次越界前明确同意；云端费用预估与预算；数据出境记录
  - [ ] 远程资源列表、删除请求状态、删除失败重试
  - [ ] API Key 系统安全存储；日志/诊断包自动脱敏
  - [ ] 隐私设置修改即时生效测试；增费/上传/建远程资源不得静默执行

## 阶段十：P1 可观测/离线/故障自愈
- [ ] Task 12: 可观测体系
  - [ ] 前端/Tauri/Sidecar correlation ID；Sidecar stdout/stderr 日志捕获；滚动与大小限制
  - [ ] Provider 耗时/错误分类；音频/TTS/avatar 阶段指标
  - [ ] 用户可理解诊断页；一键脱敏诊断 ZIP；复制诊断摘要
  - [ ] 自动检测端口占用/模型未启动/凭证错误/权限不足/磁盘不足并一键修复
- [ ] Task 13: 故障自愈
  - [ ] 离线请求队列、指数退避、熔断器、Provider fallback
  - [ ] 崩溃后恢复未完成消息；防重复模型调用/保存
  - [ ] 更新失败回滚、迁移失败恢复
  - [ ] 用户可见备份列表、选择备份恢复、恢复前再备份、恢复失败不破坏现有数据
