...
## 阶段三：P0 真实发布与更新闭环
- [ ] Task 5: 生产更新公钥注入与通道配置
  - [ ] 检查 Tauri Updater 当前实现、配置注入与签名验证全过程（`updater.rs`、`lib.rs`、`tauri.conf.json`）
  - [ ] 禁止正式构建使用开发占位公钥；将生产更新公钥以安全、可审计、不可被运行时任意替换的方式注入
  - [ ] 配置 stable 与 beta 更新通道的真实 endpoint（fails-closed：缺失不提供更新）
- [ ] Task 6: 更新全链路实现与验证
  - [ ] 版本检查、清单签名验证、下载包校验
